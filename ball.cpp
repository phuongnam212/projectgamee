...
void Ball::update(double deltaTime, bool mouseDown, bool mousePressed, std::vector<Tile> tiles,std::vector<Hole> holes,std::vector<SDL_Rect> windRectList, std::vector<InfoWind> windList, Mix_Chunk* chargeSfx, Mix_Chunk* swingSfx, Mix_Chunk* holeSfx,bool hasWind)
{
    if (win)
    {
    ...
    for (Hole h : holes)
    {
        if (getPos().x + 5 > h.getPos().x && getPos().x + 16 < h.getPos().x + 22 && getPos().y + 5 > h.getPos().y && getPos().y + 16 < h.getPos().y + 22)
        {
    ...
    else
    {
        ...
        for (int i = 0; i < windRectList.size(); i++) {
            if (checkCollision(getFloatRect(), windRectList[i])) {
                velocity = velocity + windList[i].velocWind;
                break;
            }
        }
    }
        ...
        if (getVelocity().x > 0.0001 || getVelocity().x < -0.0001 || getVelocity().y > 0.0001 || getVelocity().y < -0.0001)
        {
            ...
            if (velocity1D > 0)
            {
                velocity1D -= (friction*deltaTime)/2;
            }
            else
            {
                velocity1D = 0;
